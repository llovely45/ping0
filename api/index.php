...
if ($request === '/risk' || $request === '/risk.php') {
    include __DIR__ . '/risk.php';
    exit;
}
// --- [新增结束] ---
...
当一个 IP 显示 为广播 IP 时，通常代表这是一个 非家庭宽带 IP，非本地 IDC 提供商 IP。">(说明?)</a>
                    ...
                    <div class="content">
                                                <?php if($broadcast_status): ?><span class="label orange" style="background: rgb(255, 170, 0);"><?php if($anycastPopTrack) {echo '泛播';} else {echo '广播';} ?> IP</span><?php else: ?><span class="label orange" style="background:limegreen;">原生 IP</span><?php endif; ?>
                                            </div>
                </div>
